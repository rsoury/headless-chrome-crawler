...
const CONNECT_OPTIONS = ["browserWSEndpoint", "ignoreHTTPSErrors"];
...
class HCCrawler extends EventEmitter {
   ...
   */
  init() {
    return this._cache.init().then(() => void this._queue.init());
  }
  ...
  queue(options) {
    each(isArray(options) ? options : [options], _options => {
      ...
      if (mergedOptions.device && !includes(deviceNames, mergedOptions.device))
  ...
  _startRequest(options, depth) {
    return this._skipRequest(options).then(skip => {
      if (skip) {
  ...
  _request(options, depth, retryCount = 0) {
    ...
    return this._newPage(options).then(crawler => {
  ...
  _followSitemap(options, depth) {
    ...
    return this._getRobot(options).then(robot =>
      Promise.resolve(
        map(robot.getSitemaps(), sitemapUrl =>
          this._getSitemap(sitemapUrl).then(xml => {
            const urls = getSitemapUrls(xml);
            each(
              urls,
              url => void this._push(extend({}, options, { url }), depth)
            );
          })
        )
      )
  ...
  _getSitemap(sitemapUrl) {
    return this._cache.get(sitemapUrl).then(value => {
      if (value) return EMPTY_TXT;
      return request(sitemapUrl)
        .then(xml => this._cache.set(sitemapUrl, "1").then(() => xml))
        .catch(error => {
    ...
    return this._cache
      ...
      .then(cachedTxt => {
        ...
        return request(robotsUrl)
          .then(txt => this._cache.set(robotsUrl, txt).then(() => txt))
          .catch(error => {
   ...
   */
  _newPage(options) {
    return this._browser.newPage().then(page => new Crawler(page, options));
  }
  ...
  _followLinks(urls, options, depth) {
    ...
    return Promise.all(
      map(urls, url => {
        ...
        return this._skipRequest(_options).then(skip => {
          if (skip) return;
